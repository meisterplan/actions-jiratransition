...
const main = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const transitionName = core_1.getInput('transition_name', { required: true });
        const serviceBaseUrl = core_1.getInput('service_base_url', { required: true });
        ...
        if (issues.length === 0) {
            ...
            core_1.info(`${issues.length} issues are matching '${searchPattern}': ${issues.join(', ')}`);
            core_1.info(`Built jql search pattern: ${jql}`);
            core_1.info(`Transitioning them to '${transitionName}'...`);
            yield axios_1.default.get(url).catch((e) => core_1.setFailed(`Failed to execute transitions:\n${e.response.data}`));
        }
    }
    catch (err) {
...
const findIssues = (searchPattern, ignorePattern, commitDepth) => {
    const commitMessageLines = child_process_1.execSync(`git log -n ${commitDepth} --no-merges --format=%B`).toString().split('\n');
