...
const findIssues = (searchPattern, commitDepth) => {
    const commitMessages = child_process_1.execSync(`git log -n ${commitDepth} --no-merges --format=%B`).toString();
    const issues = commitMessages.match(new RegExp(`${searchPattern}`, 'g')) || [];
    return issues;
};
main();
